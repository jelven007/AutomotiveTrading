...
默认证券范围由 TDX 沪深列表按 60/68、00/30 筛选，包括 302132 和 689009。
未配置 Tushare 时仅为候选集合，verification 为 unverified。配置 Token 后使用
`stock_basic(list_status=L)` 校准全集；Token 不进日志或报告。

tdxpy 0.2.7 对 market=2 的列表/快照不支持，北交所必须标记缺口；即使拿到 Tushare
北交所名单也不能宣称已有其实时行情。源码通过显式市场编号取数，不使用会误判 920 的
mootdx 自动路由。返回证券与市场均需属于请求集合。
