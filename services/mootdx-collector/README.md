...
该列表是 MOOTDX/TDX 候选集合，不接入外部校准源，也不提供交易所权威全集保证。
...
Provider 市场编号、证券同步、采集调度和核心接收契约均已限定
SSE/SZSE。启动时会过滤持久化 universe 中的旧 BSE 状态，范围外市场不参与
应采、实采、缺口或聚合状态。历史 coverage 报告保持原始内容；
查询 API 会按沪深分项重新计算当前展示状态。

范围对齐由 Sidecar 44 项测试、核心/契约测试和真实 5,226/5,226 查询验证。
源时间、质量和候选范围状态仍须独立检查。
