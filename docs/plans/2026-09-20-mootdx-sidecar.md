...
- 沪市纳入 60/68；深市纳入 00/30（包括真实列表中的 302132）；北市识别 920 及旧 43/83/87/88 代码，且保留“候选范围未核验”状态，不能将整个新三板视为北交所。
...
- tdxpy 0.2.7 对北市列表和快照有显式不支持分支；直接协议探测 7 个节点的北市列表均超时。`get_security_count(2)` 的返回数不能代替北市列表或覆盖证明。
- 请求错误市场的 920002 时，服务器返回了不相干的 600839 零价行；必须按请求的市场和代码验证响应，不能仅依据非空应答计入覆盖。
- 沪深行情 `servertime` 为 `15:29:52.986` 等形式，包含毫秒；价格存在 `11.700000000000001` 浮点尾差。
- 未配置 Tushare Token，北市主数据和权威在市全集核验仍依赖外部可用源；采集服务必须显示该限制。
