...
本阶段交付证券枚举、全市场快照轮询、可靠待发送队列、核心接收改进和部署。
K 线仅用于辅助判定源交易日；历史回填、分笔、F10、Redis/WebSocket 和新前端页面
仍按总计划推进。
...
- 证券范围固定使用 TDX 列表按 60/68、00/30 筛选的沪深候选集合，
  不接入外部校准源，也不声称交易所权威全集。
...
- 沪深证券范围是 MOOTDX/TDX 前缀筛选的候选集合，不提供权威在市全集保证。
