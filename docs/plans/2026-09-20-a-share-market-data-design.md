...
5. 缺失显式：mootdx 不支持或无法完整获取的数据标记为缺口。
...
| 证券列表 | `stocks`、`stock_all` | 每日同步 | 沪深为主 |
| 板块 | `block`、Reader block | 每日版本化 | 依赖 TDX 文件 |
| F10 | `F10C`、`F10` | 目录哈希变化后抓取 | 沪深为主 |
...
北交所证券列表、交易日历和缺失字段由 Tushare 补齐。Provider 能力必须在
`provider_capability` 中显式登记，接口不得返回虚构空值。
...
全市场接口必须分页、限制可排序字段，并返回 `as_of`、`freshness_ms`、
...
消息包含单调递增的频道序列号。客户端发现跳号后暂停增量合并，调用 REST
获取快照，再从新序列继续。
...
- 主体为高密度虚拟表格，不使用卡片网格。
- 支持证券搜索、交易所、行业、板块、涨跌幅、成交额和质量状态筛选。
...
按约 5,400 只证券、每 3 秒一轮、每日 4 小时连续交易估算，未经快照去重时
...
3. mootdx 对北交所部分能力不完整，必须由 Tushare 补齐并显式展示覆盖。
